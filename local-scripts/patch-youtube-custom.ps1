# YouTube ReVanced - Custom Patch Selector
...
if (-not (Test-Path "revanced-cli-6.0.0-all.jar")) {
    Write-Host "ERROR: revanced-cli-6.0.0-all.jar not found!" -ForegroundColor Red
    Write-Host "Download from: https://github.com/ReVanced/revanced-cli/releases" -ForegroundColor Yellow
...
if (-not (Test-Path "patches-5.48.0.rvp")) {
    ...
    Write-Host "Download from: https://github.com/ReVanced/revanced-patches/releases" -ForegroundColor Yellow
    Read-Host "Press Enter to exit"
    exit 1
}
...
if (-not (Test-Path "youtube-20.14.43.apk")) {
    ...
    Write-Host "Download from: https://www.apkmirror.com/apk/google-inc/youtube/" -ForegroundColor Yellow
    Read-Host "Press Enter to exit"
    exit 1
}
...
$allArgs = @("patch", "--patch-bundle", "patches-5.48.0.rvp") + $includeArgs + @("--out", "youtube-revanced-custom.apk", "youtube-20.14.43.apk")

$process = Start-Process -FilePath "java" -ArgumentList (@("-jar", "revanced-cli-6.0.0-all.jar") + $allArgs) -NoNewWindow -Wait -PassThru
...
if ($process.ExitCode -eq 0) {
    ...
    Write-Host ""
    Write-Host "Install GmsCore first: https://github.com/ReVanced/GmsCore/releases" -ForegroundColor Yellow
    Write-Host ""
}
else {
    Write-Host ""
